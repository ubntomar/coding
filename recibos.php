...
$link = mysql_connect('localhost', 'redesagi', 'ubnt1234')
or die('No se pudo conectar: ' . mysql_error());
mysql_select_db('redesagi_facturacion') or die('No se pudo seleccionar la base de datos');
...
mysql_query($queryupdate) or die('Consulta fallida: ' . mysql_error());

$ultimo_recibo="update redesagi_facturacion.afiliados set recibo_generado='".$month_name."'  WHERE id='".$id."'";
//mysql_query($ultimo_recibo) or die('Consulta fallida: ' . mysql_error());
...
$sql = "SELECT * FROM `factura`  WHERE `factura`.`id-afiliado`='$id'   ORDER BY `factura`.`id-factura` DESC";//ojooo se puso asc temporalmente
$ultimopago="";
$pagoAnterior="";
